...
| # | Scenario | Complexity | Status | What It Teaches |
|---|----------|-----------|--------|----------------|
| **1** | [**Intro — Time Zone Agent**](scenario-1-intro/) | 🟢 Beginner | ✅ Complete | The hosted agent pattern: one function tool, hosting adapter, local test, deploy. **Start here.** |
| **2** | [**Data Crunch Agent**](scenario-2-data-crunch/) | 🟡 Intermediate | 🟡 Beta (75%) | Multi-tool agent with Blazor frontend and .NET Aspire orchestration. CSV analysis with real statistics. |
| **3** | [**Image Generator Agent**](scenario-3-image-gen/) | 🔴 Advanced (GPU) | 🚧 WIP (40%) | GPU-powered image generation using Stable Diffusion and FLUX.2. Serverless GPU workload profiles. |
| **4** | [**Code Metrics Agent**](scenario-4-code-metrics/) | 🟢 Beginner | 🚧 WIP | Pure server-side code analysis: complexity, line counts, maintainability index. Multi-tool agent with no external dependencies. |

> 📖 **Future Scenarios?** See [docs/SCENARIOS.md](docs/SCENARIOS.md) for the full scenario design catalog, including three additional proposed scenarios and their status.
